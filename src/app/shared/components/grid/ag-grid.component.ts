...
} from '../find-and-replace-angular/find-and-replace-angular.component';
import {
	AgGridComponentData,
	AgGridFilterModel,
...
import { ActionIconComponent } from './renderers/action-icon/action-icon.component';
import { DateTimeComponent } from './renderers/date-time/date-time.component';
import { FileArrayComponent } from './renderers/file-array/file-array.component';
import { HtmlRendererComponent } from './renderers/html-renderer/html-renderer.component';
import { IconValueComponent } from './renderers/icon-value/icon-value.component';
import { NotificationLinkComponent } from './renderers/notification-link/notification-link.component';
import { RoutedIconComponent } from './renderers/routed-icon/routed-icon.component';
import { RoutedLinkComponent } from './renderers/routed-link/routed-link.component';
import { YesOrNoComponent } from './renderers/yes-or-no/yes-or-no.component';
import {
	groupBy,
	map as _map,
	omit,
...
import { HeaderDialogService } from '../header-dialog/header-dialog.service';
...
import { HeaderComponent } from './renderers/header/header.component';
...
export class Datasource implements IServerSideDatasource {
	...
	constructor(private _comp: AgGridComponent, private _api: ApiService) { }
			...
			if (req.sortModel) {
				req.sortModel.forEach((r: { colId: string; sort: string }) => {
					if (
						...
						const key =
							AgGridComponent.makeValueFormatterKey(colName),
							method = sortOrder;
				...
				// req.sortModel.forEach((r: { colId: string; sort: string }) => {
				// 	const key = AgGridComponent.makeValueFormatterKey(r.colId),
...
export class AgGridComponent<T = any> implements OnInit, OnDestroy {
	...
	private _minimizeAdd() {
		...
		node.style.height = `${rowHeight * AgGridComponent.MIN_ADD_ROWS + toolbarHeight + 2
			}px`;
	}
	...
	editReport() {
		this.router.navigate([
			this._routing.editReportView(this.slice.parent, this.slice.id)
		]);
	}
	...
	ngOnInit() {
		...
		combineLatest([
			...
			.pipe(
				takeWhile(() => !this.destroyed),
				switchMap(([auth, params, args, preview]) => {
					if (!preview) {
						this.args = args;
						const id = args.slice || params.get('id');
						if (typeof id === 'string' && +id !== parseInt(id)) {
							this.namedSlice = id;
						}
						if (id) {
							return this._slices.fetch(id, {
	...
	onAddGridReady(evt: GridReadyEvent) {
		...
		evt.columnApi.setColumnVisible(AgGridComponent._DELETE_ROW_ID, true); // ensure our 'delete' is up
	...
	getAgHeaderContainerWidth(className){
		// path to apply container width
		const container = (document.querySelector('.'+className) as any);
		if(container){
			return container.clientWidth + 90 + 'px';
		}
	}
	...
	private _createwNewRow() {
		const id = --AgGridComponent.ADD_ID,
			slice = this.slice,
	...
	private _serializedColumnToCol(
		...
		if (col.valueFormatter) {
			const formulaKey = AgGridComponent.makeValueFormatterKey(id);
	...
	private _deserialize(serialized: GridSerialized = {}) {
		...
		if (this.slice.name == 'Notifications') {
			...
			let changedOrderCols = ['type', 'toRef', 'subject', 'fromRef', 'sent', "_attachments"] as any;
				...
				switch (this.columns[i].colId) {
					case 'type':
						...
						changedOrderCols[0].cellStyle =
						{
							...
							changedOrderCols[0].headerClass = "notification-first-header";
						break;
					case 'toRef':
					...
					case 'sent':
						...
						changedOrderCols[4].cellRendererFramework = this.components.dateTime;
	...
	openRelatedSlice(r) {
		this._slices.fetch(r.fromSlice).subscribe((t) => {
			...
			let url = this.router
				...
				.toString();
			this.router.navigateByUrl(url + '?returnURL=' + this.router.url);
		});
	}
	...
	private _buildColumnsFromFids(slice: Slice, ser?: GridSerialized) {
		const cols = (this.columns = []),
			...
			filters = AgGridComponent.columnFilters,
			self = AgGridComponent,
			app = this.app,
