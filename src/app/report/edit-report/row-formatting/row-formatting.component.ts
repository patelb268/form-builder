...
@Component({
	selector: 'app-row-formatting',
	templateUrl: './row-formatting.component.html',
	styleUrls: ['./row-formatting.component.scss']
})
export class RowFormattingComponent implements OnInit {
	...
	fontWeightValues = [
		{ value: 'Bold', modelName: 'bold', class: 'bold' },
		{ value: 'Light', modelName: 'light', class: 'light' },
		{ value: 'Normal', modelName: 'normal', class: '' },
		{ value: 'Default', modelName: 'default', class: '' }
	];
	fontStyleValues = [
		{ value: 'Italic', modelName: 'italic', class: 'italic' },
		{ value: 'Normal', modelName: 'normal', class: '' },
		{ value: 'Default', modelName: 'default', class: '' }
	];
	textDecorationValues = [
		{ value: 'Strikeout', modelName: 'line-through', class: 'strike-out' },
		{ value: 'None', modelName: 'none', class: '' },
		{ value: 'Overline', modelName: 'overline', class: 'overline' },
		{ value: 'Underline', modelName: 'underline', class: 'underline' },
		{ value: 'Default', modelName: 'default', class: '' }
	];
	animationValues = [
		{ value: 'Blink', modelName: 'blink' },
		{ value: 'Fixed', modelName: 'fixed' },
		{ value: 'Pulse', modelName: 'pulse' }
	];
	...
	addFormattingRow() {
		this.rowFormatting.push({
			backgroundColorCtrl: new FormControl('#ffffff'),
			textColorCtrl: new FormControl('#000000'),
			fontSizeCtrl: new FormControl('12'),
			fontWeightCtrl: new FormControl('default'),
			fontStyleCtrl: new FormControl('default'),
			textDecorationCtrl: new FormControl('default'),
			animationCtrl: new FormControl('fixed'),
			mainCondition: this.andOrOpts[0],
			whereIterations:[]
		});
		// this.addIterationParent();
	}

	moveItem(from, to) {
		var f = this.rowFormatting.splice(from, 1)[0];
		this.rowFormatting.splice(to, 0, f);
	}

	deleteSortRow(index) {
		this.rowFormatting.splice(index, 1);
	}
	...
	removeIteration(group, parent, index) {
		if (this.rowFormatting[index].whereIterations.length) {
			this.getAndRemoveMatchedObject(group, index);
		}
		if (!this.rowFormatting[index].whereIterations.length) {
			this.rowFormatting[index].whereIterations = [
				{
					name: 'test',
					control: new FormControl(),
					controlValue: new FormControl(),
		...
		this.setParent(this.rowFormatting[index].whereIterations, null);
	}

	addIteration(itr, index, parentIdx) {
		if (itr.children) {
			itr.children.push({
				name: 'd' + Math.random(),
				control: new FormControl(),
	...
	addIterationParent(index) {
		this.rowFormatting[index].whereIterations.push({
			name: 'd' + Math.random(),
			control: new FormControl(),
			controlValue: new FormControl(),
			expression: new FormControl(),
	...
	addConditionDropDown(group = null, index) {
		if (!group) {
			this.rowFormatting[index].whereIterations = [
				{
					name: 'd' + Math.random(),
			...
			this.getAndUpdateMatchedObject(group, index);
		}
		this.setParent(this.rowFormatting[index].whereIterations, null);
	}

	getAndUpdateMatchedObject(group, index) {
		let matchedObject = null;
		this.rowFormatting[index].whereIterations.forEach((element, i) => {
			if (element === group) {
				this.rowFormatting[index].whereIterations[i] = {
					name: 'd' + Math.random(),
					isCondition: true,
					conditionControl: new FormControl(head(this.andOrOpts)),
					children: [group],
	...
	getAndRemoveMatchedObject(group, index) {
		let matchedObject = null;
		let isAllParentConditional = false;
		this.rowFormatting[index].whereIterations.forEach((element, i) => {
			if (element === group) {
				this.rowFormatting[index].whereIterations.splice(i, 1);
			} else if (element.children) {
				this.searchAndRemoveNested(element.children, group, index);
			}
		});
	...
	searchAndRemoveNested(element, group, index) {
		let matchedObject = null;
		element.forEach((t, i) => {
			if (t === group) {
				matchedObject = t;
				if (element.length === 1) {
					this.getAndRemoveMatchedObject(t.parent, index);
				}
				element.splice(i, 1);
			} else if (t.children) {
				this.searchAndRemoveNested(t.children, group, index);
			}
		});
		return matchedObject;
	}

	moveUpConditionDropDown(group, index) {
		this.getAndMoveUpMatchedObject(group, index);
		this.setParent(this.rowFormatting[index].whereIterations, null);
	}

	getAndMoveUpMatchedObject(group, index) {
		let matchedObject = null;
		this.rowFormatting[index].whereIterations.forEach((element, i) => {
			if (element === group) {
				this.rowFormatting[index].whereIterations[i] = {};
				...
				this.searchAndMoveUpNested(element.children, group, index);
			}
		});
	...
	searchAndMoveUpNested(elements, group, index) {
		let matchedObject = null;
		elements.forEach((t, i) => {
			if (t === group) {
				if (t.parent) {
					this.removeParentAndShiftUp(t.parent, group, index);
				} else {
					this.replaceChildWithParent(parent, group, index);
				}

				matchedObject = t;
			} else if (t.children) {
				this.searchAndMoveUpNested(t.children, group, index);
			}
		});
		return matchedObject;
	}

	removeParentAndShiftUp(parent, element, index) {
		if (parent.parent) {
			this.addToSuperParent(parent.parent, element, index);
			this.getAndRemoveMatchedObject(parent, index);
		} else {
			this.replaceChildWithParent(parent, element, index);
		}
	}

	replaceChildWithParent(parent, element, index) {
		let matchedObject = null;
		let isAllParentConditional = false;
		this.rowFormatting[index].whereIterations.forEach((e, i) => {
			if (e === parent) {
				this.rowFormatting[index].whereIterations[i] = element;
			} else if (e.children) {
	...
	addToSuperParent(superParent, element, index) {
		let matchedObject = null;
		let isAllParentConditional = false;
		this.rowFormatting[index].whereIterations.forEach((e, i) => {
			if (e === superParent) {
				this.rowFormatting[index].whereIterations[i].children
					? this.rowFormatting[index].whereIterations[
							i
	...
	setRowFormattingConditions(rowFormattingIterations) {
		if (rowFormattingIterations.length) {
			rowFormattingIterations.forEach((element) => {
				console.log(element);
				this.rowFormatting.push({
					backgroundColorCtrl: new FormControl(
						element.backgroundColor
					),
					textColorCtrl: new FormControl(element.color),
					fontSizeCtrl: new FormControl(element.fontSize),
					fontWeightCtrl: new FormControl(element.fontWeight),
					fontStyleCtrl: new FormControl(element.fontStyle),
					textDecorationCtrl: new FormControl(element.textDecoration),
					animationCtrl: new FormControl(element.blink),
					whereIterations: element.rule[1]
						? this.getWhereIterationByData(element.rule[1])
						: [],
					mainCondition: this.getConditionField(element.rule[0])
				});
				this.whereIterations = [];
				// this.mainCondition = this.getConditionField(element.rule[0]);
			});
		}
	}

	getWhereIterationByData(rules) {
		rules.forEach((e) => {
		...
		return this.nestedWhereGenerator;
	}
	...
	constructFieldControls(y: any[]) {
		...
		if (fields.length === 2 || fields.some((y) => y === '$empty')) {
			...
			return {
				...
				control: new FormControl({
					id: fields[2],
					f: { type: 'control_related_dropdown' }
				}),
				controlValue: new FormControl(fields[2]),
				expression: new FormControl(
					this.expressionFieldsByType['control_related_dropdown'].data.find(
						(u) => u.id === fields[0]
					)
				),
				expressionOpts: new FormControl(),
		...
		// Date
			...
			if (fields.some((y) => y === '$gt')) {
				return {
					...
					controlValue: new FormControl(fields[2]),
					expression: new FormControl(
						this.expressionFieldsByType['control_date'].data.find(
							(u) => u.id === '$gt'
						)
					),
					expressionOpts: new FormControl(fields[4]),
					messageControl: new FormControl(),

					defaultValueControl: new FormControl(),
					dateAndPromptSelectorControl: new FormControl(fields[4]),
					selectedDate: new FormControl(),
					futureOrPastDaysControl: new FormControl(0),
					selectField: new FormControl(),
					selectParameter: new FormControl(),
				...
				return {
					...
					controlValue: new FormControl(fields[2]),
					expression: new FormControl(
						this.expressionFieldsByType['control_textbox'].data.find(
							(u) => u.id === fields[0]
						)
					),
					expressionOpts: new FormControl(),
	...
	private async getAllFields(p: FetchSignature) {
		...
		if (p.form) {
			...
			this.setRowFormattingConditions(
				this.reportInfoNonParent.meta.presentation.grid.rowColor
			);
